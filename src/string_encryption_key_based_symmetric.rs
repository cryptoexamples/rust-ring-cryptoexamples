...
pub fn main() -> Result<(), ExampleError> {
    use ring::aead;
    ...
    // GENERATE key and write it to the key file
    {
        ...
        let mut keyfile = File::create(keyfile)?;
    ...
    let ciphertext_base64 : String = {
        ...
        let cipher = aead::SealingKey::new(algorithm, &key)?;
        let mut in_out = original_data.to_vec();
        in_out.resize(original_data.len() + algorithm.tag_len(), 0u8);
        let ciphertext_length = aead::seal_in_place(&cipher, &nonce, &[0; 0], &mut in_out, algorithm.tag_len())?;
    ...
    let decrypted_data : String = {
        ...
        let mut in_out = base64::decode(&ciphertext_base64)?;
        let text_bytes = aead::open_in_place(&cipher, &nonce, &[0; 0], 0, &mut in_out)?;
        String::from_utf8(text_bytes.to_vec())?
    };

    if decrypted_data.as_bytes() != original_data {
        return Err(ExampleError::AssertionMismatch());
    }
    return Ok(());
}